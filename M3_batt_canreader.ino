...
The program reads CAN messages and decodes message ID 6F2 from the Tesla Battery Management System to derive
96 cell voltages and 32 temperature sensors on the Tesla Model S battery pack.  It also calculates module voltages and 
total Pack voltage and average temperature.
jack@evtv.me
...
{
   //if (SerialUSB.available())Serial.print((char)SerialUSB.read());//checkforinput(); //If serial event interrupt is on USB port, go check for keyboard input 
   if (SerialUSB.available())checkforinput();//checkforinput(); //If serial event interrupt is on USB port, go check for keyboard input 
...
void printMenu()
{
 ...
 SerialUSB<<"  i - set interval in ms for screen print \n";
...
//if(battery.cell[3]==0)
//  {
//    SerialUSB<<"  Waiting for CAN msgID 6F2 data....\n";
//    return;
//  }
  ...
  for(int i=1;i<96;i=i+6)
    {       
      ...
      SerialUSB<<"\n";
      SerialUSB<<"  Tesla Model 3 Battery Pack Monitor - http://evtv.me & Bryan Inkster\n\n";
